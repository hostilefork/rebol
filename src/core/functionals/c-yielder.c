...
REB_R Yielder_Dispatcher(REBFRM *f)
{
  ...
  resume_body: {
    ...
    Init_Unreadable_Blank(plug);  // Replug trashes, make GC safe
    ...
    //
    assert(NOT_EVAL_FLAG(yielder_frame->out, DELEGATE_CONTROL));
  ...
  body_finished_or_threw: {
    ...
    Init_Unreadable_Blank(ARR_AT(details, IDX_YIELDER_PLUG));
