...
static void Queue_Mark_Frame_And_Priors(REBFRM *f) {
    ...
    // Should have taken care of reifying all the VALIST on the stack earlie
    ...
    for (arg = FRM_ARGS_HEAD(f); NOT_END(param); ++param, ++arg) {
        ...
        if (fulfilling) {
            ...
            // !!! Mechanism for cutting down on unnecessary GC likely needs
            // to be rethought, but this invariant was true as a first take at
            // trying to "format" some cells as not forcing GC "reification".
            //
